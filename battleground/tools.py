...
# Nothing from here is imported into sunfish.py
...
def gen_legal_moves(pos):
    ...
    for move in pos.gen_moves():
        pos1 = pos.move(move)
        # If we just checked for opponent moves capturing the king,
        # we would miss captures in case of illegal castling.
        sm = sunfish.MATE_LOWER
        if not any(pos1.value(m) >= sm for m in pos1.gen_moves()):
            yield move, pos1
